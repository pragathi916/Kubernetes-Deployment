...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($username) && !empty($password) && !empty($type) && !is_numeric($username)) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            if ($user_data['password'] === $password && $user_data['usertype'] === $type) {
                ...
                echo '<script>alert("Wrong password or user type!");</script>';
            }
        } else {
            echo '<script>alert("Username not found! Please register before login.");</script>';
