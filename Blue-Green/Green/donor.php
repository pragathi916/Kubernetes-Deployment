...
if (!mysqli_query($con, $create_table_query)) {
    echo "<script>alert('Error creating donor table: " . mysqli_error($con) . "');</script>";
}
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($check_username_result) > 0) {
        // Username exists, insert donor record
        $insert_query = "INSERT INTO donor (username, place, blood_group, contact, organ, age, usertype) 
                         VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt_insert = mysqli_prepare($con, $insert_query);
        mysqli_stmt_bind_param($stmt_insert, "sssssis", $username, $place, $blood_group, $contact, $organ, $age, $usertype);
        if (mysqli_stmt_execute($stmt_insert)) {
            ...
            echo "<script>alert('Error inserting donor details: " . mysqli_error($con) . "');</script>";
        }
