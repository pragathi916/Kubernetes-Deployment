...
<head>
  ...
  <title>Register As Patient Page</title>
  <style>
  body {
...
if(isset($_POST['submit'])) {
    ...
    // Check if username exists in patients table
    ...
    if (mysqli_num_rows($check_username_result) > 0) {
      ...
      $insert_query = "INSERT INTO patients (username, place, blood_group, contact, organ, age, usertype) VALUES (?, ?, ?, ?, ?, ?, ?)";
      $stmt = mysqli_prepare($con, $insert_query);
      mysqli_stmt_bind_param($stmt, "sssssis", $username, $place, $blood_group, $contact, $organ, $age, $usertype);
      if (mysqli_stmt_execute($stmt)) {
        echo "<script>alert('Patient details successfully inserted.');</script>";
        echo '<meta http-equiv="refresh" content="2;url=patient_details.php">';
        exit();
      } else {
